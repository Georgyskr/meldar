...
describe('parseChatGptExport', () => {
	...
	describe('error cases — these are THROWS, not return values', () => {
		...
		it('throws Error("conversations.json is not an array") when root is an object, not array', async () => {
			const file = await makeZipFile(JSON.stringify({ conversations: [] }))

			await expect(parseChatGptExport(file)).rejects.toThrow('conversations.json is not an array')
		})
