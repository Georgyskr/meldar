...
function mockTopicToolResponse(input: Record<string, unknown>) {
...
function mockGoogleTopicToolResponse(input: Record<string, unknown>) {
	mockMessagesCreate.mockResolvedValue({
		content: [
			{
				type: 'tool_use' as const,
...
describe('extractTopicsFromMessages', () => {
	...
	it('returns empty arrays when response.content has no tool_use block', async () => {
		mockMessagesCreate.mockResolvedValue({
			content: [{ type: 'text' as const, text: 'I cannot extract topics' }],
		})
...
describe('extractGoogleTopics', () => {
	...
	it('returns empty arrays when response.content has no tool_use block', async () => {
		mockMessagesCreate.mockResolvedValue({
			content: [{ type: 'text' as const, text: 'Cannot extract' }],
		})
