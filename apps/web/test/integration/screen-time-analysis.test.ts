...
// Skip when ANTHROPIC_API_KEY is missing so the default `pnpm vitest` run stays
// deterministic. Run these locally with the key set: `ANTHROPIC_API_KEY=... pnpm vitest test/integration`.
const describeIfKey = process.env.ANTHROPIC_API_KEY ? describe : describe.skip

describeIfKey('Screen Time → Adaptive pipeline (real API)', () => {
