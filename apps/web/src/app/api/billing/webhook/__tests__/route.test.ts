...
describe('POST /api/billing/webhook', () => {
	...
	describe('authorization checks (before constructEvent)', () => {
		it('returns 401 when stripe-signature header is absent', async () => {
			const req = makeNextRequest('http://localhost/api/billing/webhook', {
				method: 'POST',
				body: 'payload',
			})
	...
	describe('checkout.session.completed — appBuild product (legacy slug)', () => {
		beforeEach(() => {
			mockWebhooksConstructEvent.mockReturnValue(
	...
	describe('checkout.session.completed — vipBuild product (v3 canonical slug)', () => {
		beforeEach(() => {
			mockWebhooksConstructEvent.mockReturnValue(
				makeSessionCompletedEvent({ metadata: { product: 'vipBuild', xrayId: '' } }),
			)
		})

		it('inserts into auditOrders with product: "app_build" (db column unchanged for back-compat)', async () => {
			await POST(makeWebhookRequest('payload', 'sig'))
			const firstInsertValues = mockDbValues.mock.calls[0][0]
			expect(firstInsertValues.product).toBe('app_build')
		})

		it('sends both buyer + founder emails', async () => {
			await POST(makeWebhookRequest('payload', 'sig'))
			expect(mockResendEmailsSend).toHaveBeenCalledTimes(2)
		})
	})

	describe('checkout.session.completed — builder product (v3 subscription)', () => {
		beforeEach(() => {
			mockWebhooksConstructEvent.mockReturnValue(
				makeSessionCompletedEvent({ metadata: { product: 'builder', xrayId: '' } }),
			)
		})
		...
		it('does NOT send emails on checkout.session.completed (entitlement is granted via customer.subscription.created)', async () => {
			await POST(makeWebhookRequest('payload', 'sig'))
			expect(mockResendEmailsSend).not.toHaveBeenCalled()
		})

		it('inserts buyer into subscribers table', async () => {
			await POST(makeWebhookRequest('payload', 'sig'))
			const insertValues = mockDbValues.mock.calls[0][0]
	...
	describe('checkout.session.completed — starter product (legacy retired subscription)', () => {
		beforeEach(() => {
			mockWebhooksConstructEvent.mockReturnValue(
