...
describe('POST /api/billing/checkout', () => {
	...
	describe('Stripe session creation', () => {
		...
		it('creates a payment-mode session for product "appBuild" (legacy slug, maps to vipBuild)', async () => {
		...
		it('creates a payment-mode session for product "vipBuild" (v3 canonical slug)', async () => {
			await POST(makeRequest({ product: 'vipBuild' }))
		...
		it('creates a subscription-mode session for product "builder" (v3 EUR 19/mo)', async () => {
			await POST(makeRequest({ product: 'builder' }))
		...
		it('rejects retired "starter" slug with VALIDATION_ERROR', async () => {
			const res = await POST(makeRequest({ product: 'starter' }))
			expect(res.status).toBe(400)
			const json = await res.json()
			expect(json.error.code).toBe('VALIDATION_ERROR')
		})
		...
		it('DOES include subscription_data.trial_period_days: 7 in Stripe call for builder (subscription mode)', async () => {
			await POST(makeRequest({ product: 'builder' }))
