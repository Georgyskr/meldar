...
function mockToolUseResponse(input: Record<string, unknown>): void {
	mockMessagesCreate.mockResolvedValue(
		makeAnthropicMessage({
			content: [makeToolUseBlock('extract_screen_time', input)],
		}),
	)
}
...
describe('extractScreenTime', () => {
	...
	describe('broken AI responses', () => {
		it('throws Error("No tool use in response") when response.content has no tool_use block', async () => {
			mockMessagesCreate.mockResolvedValue(
				makeAnthropicMessage({
					content: [makeTextBlock('I cannot process this image')],
				}),
			)
