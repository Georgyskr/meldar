...
function mockToolUseResponse(toolName: string, input: Record<string, unknown>): void {
	mockMessagesCreate.mockResolvedValue(
		makeAnthropicMessage({ content: [makeToolUseBlock(toolName, input)] }),
	)
}
...
describe('extractFromOcrText', () => {
	...
	describe('error cases', () => {
		it('returns { error } when response.content has no tool_use block', async () => {
			mockMessagesCreate.mockResolvedValue(
				makeAnthropicMessage({ content: [makeTextBlock('No tool use here')] }),
			)
