...
describe('parseClaudeExport', () => {
	...
	describe('error cases — these are THROWS', () => {
		...
		// caller had no way to know it had handed in garbage. Real exports
		// in the wild include corrupted/truncated files; we'd rather fail
		// loudly than ship empty insights.

		it('throws on a root that is a primitive (string)', async () => {
			const file = makeFile(JSON.stringify('not an export'))
			await expect(parseClaudeExport(file)).rejects.toThrow(/Invalid Claude export shape/i)
		})

		it('throws on a root that is a primitive (number)', async () => {
			const file = makeFile(JSON.stringify(42))
			await expect(parseClaudeExport(file)).rejects.toThrow(/Invalid Claude export shape/i)
		})

		it('throws on a root that is null', async () => {
			const file = makeFile('null')
			await expect(parseClaudeExport(file)).rejects.toThrow(/Invalid Claude export shape/i)
		})

		it('throws when .conversations is present but not an array', async () => {
			const file = makeFile(JSON.stringify({ conversations: 'not an array' }))
			await expect(parseClaudeExport(file)).rejects.toThrow(/Invalid Claude export shape/i)
		})
	})
})
